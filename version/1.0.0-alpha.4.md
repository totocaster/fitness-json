...
| `comments` | \[comment\] |  | Post-workout comments. May contain many [comments](../#comment) from various [people](../#author). |
| `profile` | profile |  | The snapshot of the profile at the time of the workout completion.[‡](1.0.0-alpha.4.md) |
...
It is recommended to store unprocessed location data from the sensors if it is withing resonable noise to signal ratio. Smoothing and other filtering techniques can be applied after data is decoded.

#### Displacement and Cumulative Distance

Unlike most fitness file formats, fitness·json supports two types of measurements to indicate athletes movement in terms of distance. TODO: this paragphraph is incomplete.
...
Guide, or Guided Workout, is an object or file with a set of instructions of how the athlete should perform the activity. Guides are great tools for structured workouts as they can be as vague or as strict as the guide author desires.
...
Non-empty guides must have at least one interval with at least one block. Otherwise, the guide is considered to be empty. The empty guide is not suitable for activity.
...
Block can be one of _several types_, have completion condition, and optionally, target too. Targets itself, can be _one of four types_.
